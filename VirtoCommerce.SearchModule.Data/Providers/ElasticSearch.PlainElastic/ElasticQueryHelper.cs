...
namespace VirtoCommerce.SearchModule.Data.Providers.ElasticSearch.PlainElastic
{
    ...
    {
        public static BoolFilter<DocumentDictionary> CreateQuery(ISearchCriteria criteria, ISearchFilter filter)
        {
            var values = GetFilterValues(filter);
            if (values == null)
                return null;

            var query = new BoolFilter<DocumentDictionary>();
        ...
        public static BoolFilter<DocumentDictionary> CreateQueryForValue(ISearchCriteria criteria, ISearchFilter filter, ISearchFilterValue value)
        {
            var query = new BoolFilter<DocumentDictionary>();
            ...
            else
            {
                ...
                else if (value.GetType() == typeof(RangeFilterValue))
                {
                    ...
                    var tempFilter = new RangeFilter<DocumentDictionary>();
        ...
        /// <returns></returns>
        public static BoolFilter<DocumentDictionary> CreatePriceRangeFilter(ISearchCriteria criteria, string field, RangeFilterValue value)
        {
            var query = new BoolFilter<DocumentDictionary>();
            ...
            var filter = new RangeFilter<DocumentDictionary>();
        ...
        private static BoolFilter<DocumentDictionary> CreatePriceRangeFilter(string[] priceLists, int index, string field, string currency, string lowerbound, string upperbound, bool lowerboundincluded, bool upperboundincluded)
        {
            var query = new BoolFilter<DocumentDictionary>();
            ...
            {
                var filter2 = new RangeFilter<DocumentDictionary>();
